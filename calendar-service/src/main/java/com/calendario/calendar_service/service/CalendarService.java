...
import com.calendario.calendar_service.client.UserFeignClient;
...
public class CalendarService {
    ...
    // Constructor
    public CalendarService(CalendarRepository calendarRepository, UserFeignClient userService,
    ...
    public CalendarResponseDTO createCalendar(CalendarCreateRequestDTO dto, String authenticatedUsername) {
        ...
        UserResponseDTO authenticatedUser;
        try {
            authenticatedUser = userService.findByUsername(authenticatedUsername);
        ...
        List<String> userCalendarIds = userService.getCalendarIdsForUser(authenticatedUser.getId());
        ...
        userService.addCalendarMembership(authenticatedUser.getId(),
                new CalendarMembershipDTO(savedCalendar.getId(), true));
//        userService.saveUser(authenticatedUser); //no longer necessary since userservice.addcalendarmembership alters the user
    ...
    public CalendarUpdateResponseDTO updateCalendar(String calendarId, CalendarUpdateRequestDTO dto, String authenticatedUsername) {
        ...
        if (!isUserAdminOfCalendar(authenticatedUser.getId(), calendarId)) {
        ...
        if (dto.getAdmins() != null && !dto.getAdmins().isEmpty()) {
            for (String userId : dto.getAdmins()) {
                ...
                } catch (FeignException e) { continue; }
                if (user != null) {
                    if (user.isMemberOfCalendar(calendarId)) {
                        // Find and update their membership to admin
                        userService.addCalendarMembership(
                                user.getId(),
                                new CalendarMembershipDTO(updatedCalendar.getId(), true)
                        );
                    }
                }
            }
    ...
    private boolean isUserAdminOfCalendar(String userId, String calendarId) {
        try {
            CalendarMembershipDTO membership =
                    userService.checkMembership(userId, calendarId);
